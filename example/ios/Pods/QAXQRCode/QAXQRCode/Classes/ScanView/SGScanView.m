...
- (instancetype)initWithConfigure:(SGScanViewConfigure *)configure
{
    self = [super init];
    if (self) {
...
//    CGRect frame = CGRectMake(ceilf(x), ceilf(y), ceilf(w), ceilf(h));;
    
    
    CGFloat minSide = MIN(self.frame.size.width, self.frame.size.height);

    CGFloat sideLength = minSide * 0.7;

    CGFloat x = self.frame.size.width / 2 - sideLength / 2;
    CGFloat y = self.frame.size.height / 2 - sideLength / 2;
    
    _borderFrame =CGRectMake(x, y, sideLength, sideLength);
    [self setScanFrame:self.frame];
...
//        }
        _scanlineImgView.image = [QAXSpecResoursLoader imageWithName:self.configure.scanline];
        
        if (_scanlineImgView.image) {
...
- (void)orientChange:(UIInterfaceOrientation)orientation{
